...
- **Convite automático pra novo admin + sessão presa após remoção (2026-07-22)**: `addAdmin()` (`lib/actions/admin.ts`) antes só criava a linha na tabela — a pessoa não sabia que tinha acesso até ela mesma ir em `/admin/login`. Agora chama `supabase.auth.admin.inviteUserByEmail()` na hora de adicionar, preenchendo `supabaseUserId` da resposta. Caso o e-mail já tenha usuário Supabase (readicionar alguém removido antes, ou quem já tinha tentado logar sozinho antes desse convite existir), `inviteUserByEmail` falha com `422 email_exists` — nesse caso cai num fallback pra `signInWithOtp` (manda Magic Link em vez de Invite) buscando o id existente direto em `auth.users` via SQL (não via `generateLink()` do Admin API — chamar `generateLink` e depois `signInWithOtp` pro mesmo e-mail em sequência bate no cooldown de 60s do Supabase por identidade, reproduzido ao vivo). Achado relacionado: remover um admin (`removeAdmin`) apagava só a linha do Postgres, nunca o usuário Supabase nem a sessão dele — se a pessoa removida ainda estivesse logada, ficava presa num loop (`/admin` → sem registro → `/?deniedAccess=1`, mas a própria `/admin/login` achava que ela "já tava logada" pela sessão ainda válida e mandava de volta pro loop) até limpar cookie manualmente. Corrigido: `lib/require-admin.ts` no caso `forbidden` agora redireciona pra um Route Handler novo (`app/admin/access-denied/route.ts`) que chama `signOut()` de verdade antes do redirect final — precisa ser Route Handler, não Server Component, porque só lá dá pra escrever cookie (`lib/supabase/server.ts` engole em silêncio qualquer tentativa de `setAll` fora de Server Action/Route Handler). Testado de ponta a ponta por subagente: sessão de admin removido gera cookie `sb-*-auth-token` vazio após passar por `/admin`, e `/admin/login` volta a mostrar o form de verdade.
