...
| Lint | oxlint | 1.74.0 |
| ORM | Prisma | 7.8.0 — driver adapter obrigatório em runtime, `schema.prisma` sem `url`/`directUrl` (ver §8, comportamento real confirmado na Fase 0, não é o Prisma 5/6 que a memória de treinamento assume) |
...
- **Supabase Auth** autentica os ~3 admins por **magic link** (decidido e implementado na Fase 0, ver §7/§9 do PRD). **Supabase Storage** guarda as imagens de todas as seções (bucket público para leitura, escrita restrita a `service_role`/admin autenticado).
...
| Rodapé | custom + ícones | ícones sociais do protótipo são SVGs desenhados à mão (Instagram/YouTube/WhatsApp) — **desvio justificado**: usar ícones de um pacote de componentes em vez de recriar os SVGs. **Estado final da Fase 1** (ver nota completa no §2): `lucide-react` foi removido do projeto inteiro (não só o rodapé — cobria bem os 3 ícones de marca no começo, mas parou de exportar `Instagram`/`Youtube`/`Whatsapp` na versão instalada, e duas tentativas de recriar os glifos à mão saíram malformadas). Solução final: **só `react-icons`** — `react-icons/fa` (`FaInstagram`, `FaYoutube`, `FaWhatsapp`) pros 3 ícones de marca aqui no rodapé, `react-icons/lu` pra todo ícone genérico do resto do site. Manter tamanho 40×40px circular e hover azul nos 3. |
