...
| Rodapé | custom + ícones | ícones sociais do protótipo são SVGs desenhados à mão (Instagram/YouTube/WhatsApp) — **desvio justificado**: usar `lucide-react`, já que é a `iconLibrary` configurada em `components.json`. **Atualização Fase 1**: `lucide-react` 1.25 removeu todos os ícones de marca/logo (confirmado empiricamente — não há `Instagram`, `Youtube`, `Whatsapp`, `Twitter`, `Facebook` etc. nos exports do pacote instalado). Duas tentativas de recriar os glifos como SVG inline à mão saíram malformadas (curvas quebradas, ilegíveis em 16px). Solução final: `react-icons` (`react-icons/fa` — `FaInstagram`, `FaYoutube`, `FaWhatsapp`) só para esses 3 ícones de marca em `components/footer/footer.tsx`; `lucide-react` continua sendo a `iconLibrary` pra todo o resto do site (Calendar, MapPin, Play, Check, Copy, Menu, etc.) — não é uma substituição geral da lib, é uma exceção pontual pra ícones de marca que o lucide não cobre mais. Manter tamanho 40×40px circular e hover azul nos 3. |
